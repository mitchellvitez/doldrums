...
data StgAlt = StgAlt Tag [Name] StgExpr
  deriving (Show)
...
toStg e = case e of
  ExprInt n -> pure . StgAtom . StgAtomLit $ StgLitInt n
  ExprString s -> pure . StgAtom . StgAtomLit $ StgLitString s
  ExprDouble d -> pure . StgAtom . StgAtomLit $ StgLitDouble d
  ExprVariable v -> pure . StgAtom . StgAtomVar $ unName v
  ExprConstructor tag _ -> pure . StgAtom . StgAtomVar $ unTag tag
  ExprLet name bind body -> do
    stgBody <- toStg body
    case bind of
      lam@(ExprLambda {}) -> do
        let (args, body') = collectLambdas lam
        stgBody' <- toStg body'
        pure $ StgLet [StgBinding (unName name) [] (map unName args) stgBody'] stgBody
      _ -> do
        stgBind <- toStg bind
        pure $ StgLet [StgBinding (unName name) [] [] stgBind] stgBody
...
toStgAlt (Alternative tag names body) = do
  stgBody <- toStg body
  pure $ StgAlt tag names stgBody

atomize :: Expr -> State Int (StgAtom, [StgBinding])
atomize = \case
  ExprVariable v -> pure (StgAtomVar (unName v), [])
  ExprInt n -> pure (StgAtomLit (StgLitInt n), [])
  ExprString s -> pure (StgAtomLit (StgLitString s), [])
  ExprDouble d -> pure (StgAtomLit (StgLitDouble d), [])
  ...
  where
    annotateBinding binding =
      let localBound = Set.union bound (Set.fromList (stgArgs binding))
          fvs = Set.toList
                  $ Set.difference (freeVarsExpr localBound (stgBody binding)) bound
      in binding { stgFreeVars = fvs }

    annotateAlt (StgAlt tag names body) =
      let altBound = Set.union bound (Set.fromList (map unName names))
      in StgAlt tag names (annotateFreeVars altBound body)
...
freeVarsExpr bound = \case
  ...
  StgCase atom alts ->
    freeVarsAtom atom `Set.difference` bound
      <> foldMap f alts
    where
      f (StgAlt _ names body) =
        freeVarsExpr (Set.union bound (Set.fromList (map unName names))) body
